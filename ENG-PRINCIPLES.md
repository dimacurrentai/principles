...
- **Agents provide values; shipped code serializes structured output.** A skill that promises JSON, YAML, XML, CSV, or another machine-readable format ships a short deterministic writer under its own `scripts/` directory and instructs the agent to call it with ordinary scalar flags. Show a complete, copy-pasteable invocation with multiple flags and records, including the shell escaping required for quotes: `python3 scripts/write_result.py --grade="good" --issue --file="src/parser.py" --line="17" --description="The parser returns \"ok\" before validation." --issue --file="README.md" --line="9" --description="Document the \"strict\" mode."`. The script validates types and allowed values, constructs the complete document in memory with the language's standard serializer, and atomically writes the declared output path. Never ask an agent to hand-author structured syntax, interpolate values into a JSON/YAML shell string, generate a serializer on the fly, or validate its own improvised serialization. Test both the documented invocation and the shipped writer directly with quotes, backslashes, newlines, Unicode, empty values, multiple arguments, and repeated records. This applies equally to final results and intermediate artifacts: schemas constrain data, while reviewed code owns encoding.
