...
- **Run shipped scripts by absolute path; preserve the task working directory.** Deliver the complete skill directory and state the installed `SKILL.md` path to the agent in its prompt. Resolve scripts relative to that file, invoke their interpreter explicitly, and do not `cd` into the skill. Scripts locate bundled resources relative to their own file; task-relative paths remain relative to the working directory. Do not depend on harness-specific variables. If a script is not found, re-resolve it against the `SKILL.md` directory; if it is truly absent, report the failure — never recreate it.

- **Agents provide values; shipped code serializes structured output.** A skill that promises JSON, YAML, XML, CSV, or another machine-readable format ships a short deterministic writer under its own `scripts/` directory and instructs the agent to call it with ordinary scalar flags, by the writer's resolved path and from the working directory the task already runs in — never by `cd`-ing into the skill, which would move every relative output path. Show a complete, copy-pasteable invocation with multiple flags and records. Values go in **single quotes**, where the shell expands nothing; the one escape is `'\''` for an apostrophe. Never document double-quoted values: agent prose is full of backticks and `$`, and inside double quotes the shell runs them as command substitution and variable expansion before the writer sees a byte. Set `SKILL_DIR` explicitly to the absolute directory containing the loaded `SKILL.md` before running: `python3 "$SKILL_DIR/scripts/write_result.py" --grade='good' --issue --file='src/parser.py' --line='17' --description='The parser returns "ok" before validating $input; the caller'\''s error is lost.' --issue --file='README.md' --line='9' --description='Document the "strict" mode.'`. The script validates types and allowed values, constructs the complete document in memory with the language's standard serializer, and atomically writes the declared output path. Never ask an agent to hand-author structured syntax, interpolate values into a JSON/YAML shell string, generate a serializer on the fly, or validate its own improvised serialization. Test the documented invocation verbatim through a real shell, from a working directory other than the skill's, and the shipped writer directly, with quotes, apostrophes, backslashes, backticks, `$` and `$(…)` text, newlines, Unicode, empty values, multiple arguments, and repeated records. A skill that otherwise forbids execution says in so many words that running its own shipped writer is the one exception. This applies equally to final results and intermediate artifacts: schemas constrain data, while reviewed code owns encoding.
...
## 15. Workarounds & Constraint Hygiene

- **A workaround is a claim about the world — date it, source it, and give it an expiry.** Any constraint added because of an external circumstance — a tool's concurrency ceiling, a flaky dependency, an incident, a demo that must not flake — is not a design decision; it is a perishable fact. The comment at the site states all three: what fact forced it, what evidence established that fact and when, and what observation would justify removing it. "Tool X currently sustains three concurrent sessions reliably," with no date, no measurement, and no removal condition, is a fossil in the making.

- **Never enshrine a workaround in a test.** Tests pin behavior we *want*; a test asserting the hedge itself ("the pipeline must open exactly three lanes") promotes a temporary concession into a permanent requirement and turns every future cleanup red — the exact opposite of a test's job. If the workaround needs a guard, guard the *symptom* it prevents, or mark the test explicitly as expiring together with the workaround it protects.

- **Copying or promoting code re-adjudicates every constraint it carries.** Demo → product, prototype → library, one repo → another: promotion is a review boundary. Each inherited hedge either re-justifies itself against the new context or is dropped at the boundary. What made a demo robust is not automatically what the product needs, and a rationale written for the old context is wrong by default in the new one.

- **A sibling that ignores the rule refutes the rule.** When one code path enforces a limit while a sibling path freely violates it without harm — a throttled workflow next to an unthrottled flat run hitting the same backend — the limit is contradicted by the system's own behavior. Treat such contradictions as removal triggers, not curiosities.

- **"Why is it this way?" must be answerable without archaeology.** If recovering a rule's rationale takes a walk through git history, three commits, and a deleted file, the rationale was never recorded where it lived. §1 demands a comment on every field; the same bar applies to every constraint: the code states its own justification, and when the justification expires, the constraint goes with it.
