...
- **Agents provide values; shipped code serializes structured output.** A skill that promises JSON, YAML, XML, CSV, or another machine-readable format ships a short deterministic writer under its own `scripts/` directory and instructs the agent to call it with ordinary scalar flags such as `--description="Properly escaped value"`. The script validates types and allowed values, constructs the complete document in memory with the language's standard serializer, and atomically writes the declared output path. Never ask an agent to hand-author structured syntax, interpolate values into a JSON/YAML shell string, generate a serializer on the fly, or validate its own improvised serialization. Test the shipped writer directly with quotes, backslashes, newlines, Unicode, empty values, and repeated records. This applies equally to final results and intermediate artifacts: schemas constrain data, while reviewed code owns encoding.
